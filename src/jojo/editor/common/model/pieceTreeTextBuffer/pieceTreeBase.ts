import { CharCode } from 'jojo/base/common/charCode';
import { NodeColor, SENTINEL, TreeNode, fixInsert, leftest, righttest } from 'jojo/editor/common/model/pieceTreeTextBuffer/rbTree';
...
export class PieceTreeBase {
  ...
  private getLineRawContent(lineNumber: number): string {
    ...
    while (x !== SENTINEL) {
      if (x.left !== SENTINEL && x.lf_left >= lineNumber - 1) {
        ...
        const accumualtedValue = this.getAccumulatedValue(x, lineNumber - x.lf_left - 1);
  ...
  private iterate(node: TreeNode, callback: (node: TreeNode) => boolean): boolean {
    if (node === SENTINEL) {
