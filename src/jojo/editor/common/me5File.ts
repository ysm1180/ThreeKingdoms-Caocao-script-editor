...
import * as Convert from 'jojo/base/common/convert';
...
import { FilterFuntion, IResourceStat } from 'jojo/workbench/services/binaryfile/binaryFiles';
...
export class Me5File extends BinaryFile {
  ...
  private _setItem(
    ...
    if (saveToPngMenuItem.checked) {
      ...
      convertPromise = ImageFile.convertToPng(item.data, pngCompressOption.checked);
    } else {
      convertPromise = ImageFile.convertToJpeg(item.data);
    }
