import { addClass, addDisposableEventListener, removeClass } from 'jojo/base/browser/dom';
...
export class Audio extends Disposable {
  ...
  constructor(parent: HTMLElement) {
    ...
    this.registerDispose(addDisposableEventListener(this.playButton, 'click', () => this.onPlayPause()));
    this.registerDispose(addDisposableEventListener(this._audio, 'timeupdate', () => this.onTimeUpdate()));
    this.registerDispose(
  ...
  private onMovePlayHead(event: MouseEvent): void {
    const mouseEvent = new StandardMouseEvent(event);

    const unbind: IDisposable[] = [];

    unbind.push(
      addDisposableEventListener(this.timeline, 'mousemove', (e: MouseEvent) => {
        const mouseEvent = new StandardMouseEvent(e);

        this._audio.currentTime =
          this._audio.duration *
          ((mouseEvent.posx - this.timeline.getBoundingClientRect().left) / this.timeline.offsetWidth);
      })
    );
