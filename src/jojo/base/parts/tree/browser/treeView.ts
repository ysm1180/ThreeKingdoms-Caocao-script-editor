import { addClass, removeClass } from 'jojo/base/browser/dom';
import { StandardMouseEvent } from 'jojo/base/browser/mouseEvent';
import { ScrollableElement } from 'jojo/base/browser/ui/scrollbar/scrollbarElement';
import { ArrayIterator, IIterator } from 'jojo/base/common/iterator';
import { Disposable } from 'jojo/base/common/lifecycle';
import { ITreeContext, MouseContextMenuEvent } from 'jojo/base/parts/tree/browser/tree';
import * as Model from 'jojo/base/parts/tree/browser/treeModel';
...
export class ViewItem extends Disposable {
  ...
  public render(skipContextRender = false): void {
    if (!this.element) {
      return;
    }
...
export class TreeView {
  ...
  public setModel(model: Model.TreeModel): void {
    this.model = model;
  ...
  public onClearingRoot(item: Model.Item): void {
    if (item) {
      this.onRemoveItems(item.getNavigator());
    }
  }

  public onSetRoot(item: Model.Item): void {
    this.root = new RootViewItem(this.context, item);
  }

  public onItemAddTrait(e: Model.IItemTraitEvent): void {
  ...
  public onItemRemoveTrait(e: Model.IItemTraitEvent): void {
    const item = <Model.Item>e.item;
    const trait = <string>e.trait;
    const viewItem = this.items[item.id];
    if (viewItem) {
  ...
  public onDidExpand(e: Model.IItemExpandEvent): void {
    const item = <Model.Item>e.item;
    const viewItem = this.items[item.id];
    if (viewItem) {
  ...
  public onDidCollapse(e: Model.IItemCollapseEvent): void {
    const item = <Model.Item>e.item;
    const viewItem = this.items[item.id];
    if (viewItem) {
  ...
  public onItemRefresh(e: Model.IItemRefreshEvent): void {
    const item = <Model.Item>e.item;
    const viewItem = this.items[item.id];
    this.refreshViewItem(viewItem);
  }

  public onItemChildrenRefresing(e: Model.IItemChildrenRefreshEvent): void {
    const item = <Model.Item>e.item;
  ...
  public onItemChildrenRefreshed(e: Model.IItemChildrenRefreshEvent): void {
    const item = <Model.Item>e.item;
