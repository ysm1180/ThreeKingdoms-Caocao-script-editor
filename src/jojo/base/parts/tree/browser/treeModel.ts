import { ChainEventStorage, Event } from 'jojo/base/common/event';
import { IIterator } from 'jojo/base/common/iterator';
import { IDisposable, dispose } from 'jojo/base/common/lifecycle';
import { ITreeContext } from 'jojo/base/parts/tree/browser/tree';

interface IMap<T> {
  [id: string]: T;
}
type IItemMap = IMap<Item>;

export interface IBaseItemEvent {
  item: Item;
}
export type IItemRefreshEvent = IBaseItemEvent;
export interface IItemChildrenRefreshEvent extends IBaseItemEvent {
  skip: boolean;
}
export type IItemExpandEvent = IBaseItemEvent;
export type IItemCollapseEvent = IBaseItemEvent;
...
export class Item {
  ...
  private refreshChildren(skipRenderChildren = false, force = false): Promise<any> {
  ...
  public refresh(skipRenderChildren = false): Promise<any> {
  ...
  public getNavigator(): ItemNavigator {
    ...
    if (this.item) {
      do {
        if (
          ...
          (this.item.isVisible() && this.item.isExpanded() && this.item.firstChild)
        ) {
...
export class TreeModel {
  ...
  public refresh(element: any = null, skipRenderChildren = false) {
