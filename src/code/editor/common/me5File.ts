...
import {
    FilterFuntion, IResourceStat
} from '../workbench/services/resourceFile/resourceDataService';
...
export class Me5File extends BinaryFile {
    ...
    constructor(resource: string | Buffer) {
        ...
        if (isBuffer(resource)) {
            this.itemInfoStartOffset =
                this.getGroupCount() * Me5File.GROUP_HEADER_SIZE +
                Me5File.GROUP_INFO_START_OFFSET;
        }
    }
    ...
    private _getFirstItemIndexInGroup(groupIndex: number) {
        return this.readNumber(
            Me5File.GROUP_INFO_START_OFFSET +
                4 +
                groupIndex * Me5File.GROUP_HEADER_SIZE
        );
    }
    ...
    public getGroupItemCount(groupIndex: number) {
        return (
            this._getLastItemIndexInGroup(groupIndex) -
            this._getFirstItemIndexInGroup(groupIndex) +
            1
        );
    }
    ...
    private _getOffsetByItemIndex(itemIndex: number): number {
        return this.readNumber(
            this.itemInfoStartOffset + itemIndex * Me5File.ITEM_HEADER_SIZE
        );
    }
    ...
    public getItemNameLength(groupIndex: number, subItemIndex: number) {
        ...
        return this.readNumber(
            this.itemInfoStartOffset +
                4 +
                (firstItemIndex + subItemIndex) * Me5File.ITEM_HEADER_SIZE
        );
    }
    ...
    public async save(
        stat: IResourceStat,
        groupFilter?: FilterFuntion<IResourceStat>
    ) {
        return new Promise<void>((c1, e) => {
            fs.open(this._path, 'w', (openError, fd) => {
                ...
                const itemLengths = groups.map(
                    group => group.getChildren().length
                );
                this._setAllItemCount(
                    itemLengths.reduce((pre, cur) => pre + cur, 0)
                );
                this._setGroupCount(groups.length);

                this.itemInfoStartOffset =
                    groups.length * Me5File.GROUP_HEADER_SIZE +
                    Me5File.GROUP_INFO_START_OFFSET;
                ...
                let offset =
                    this.itemInfoStartOffset +
                    baseItemIndex * Me5File.ITEM_HEADER_SIZE;
                baseItemIndex = 0;
    ...
    private _setGroupInfo(
        ...
        const offset =
            Me5File.GROUP_INFO_START_OFFSET +
            group.getIndex(filter) * Me5File.GROUP_HEADER_SIZE;
        this.writeInt(offset + 4, baseItemIndex); // Item Begin Index
        this.writeInt(
            offset + 8,
            baseItemIndex + group.getChildren().length - 1
        ); // Item End Index
    ...
    private _setItemInfo(
        ...
        const offset =
            this.itemInfoStartOffset +
            (baseItemIndex + item.getIndex(filter)) * Me5File.ITEM_HEADER_SIZE;
    ...
    private _setItem(
        ...
        if (saveToPngMenuItem.checked) {
            const pngCompressOption = appMenu.getMenuItemById(
                'option.maxCompressPng'
            );
            convertPromise = ImageResource.convertToPng(
                item.data,
                pngCompressOption.checked
            );
        } else {
