...
export class FileService implements IFileService {
    ...
    private fillContents(
        content: IStreamContent,
        resource: string,
        encoding?: string
    ) {
        return this.resolveFileData(resource, encoding).then(
            (data: IContentData) => {
                    ...
                    fs.read(
                        ...
                        (err, bytesRead) => {
                            if (decoder) {
                                ...
                                result.stream = decoder = iconv.decodeStream(
                                    encoding
                                );
                                resolve(result);
                                handleChunk(bytesRead);
